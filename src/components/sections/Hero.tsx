...
const HeroClientCard: React.FC<{ className?: string }> = ({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-[20px] sm:rounded-[24px] lg:rounded-[28px] border border-white/10 bg-gradient-to-br from-[#0f1d3d]/75 via-[#0c3c93]/70 to-[#0a7bff]/75 px-4 py-3.5 sm:px-7 sm:py-7 lg:px-9 lg:py-8 shadow-[0_10px_60px_rgba(10,123,255,0.35)] backdrop-blur-lg ${className}`}
    >
      ...
      <div className="relative space-y-2.5 sm:space-y-5 text-white">
        <h2 className="text-lg sm:text-3xl font-semibold leading-snug">
          <span className="block">Mova-se mais rápido.</span>
          <span className="block text-[#51d6ff]">Pense grande.</span>
        </h2>
        <p className="hidden sm:block text-sm sm:text-base text-white/85 max-w-md">
          Transformando negócios com soluções de ponta
        </p>
        <div className="flex items-center gap-2.5 sm:gap-4 pt-0.5 sm:pt-2">
          <div className="flex -space-x-2 sm:-space-x-3">
            {HERO_AVATARS.map((src, idx) => (
              <span
                key={src}
                className="inline-block h-8 w-8 sm:h-12 sm:w-12 rounded-full border-2 border-white/70 shadow-md overflow-hidden bg-white/10"
                style={{ zIndex: 3 - idx }}
              >
                <img
                  src={src}
          ...
          <div className="leading-tight">
            <div className="text-lg sm:text-2xl font-semibold">1.000 mil</div>
  ...
  return (
    <section className="relative min-h-[100svh] pt-32 sm:pt-28 lg:pt-20 pb-12 sm:pb-24 lg:pb-14 flex items-start lg:items-center overflow-hidden">
      <div className="pointer-events-none absolute inset-0 z-0">
        ...
        <div className="absolute inset-x-0 bottom-0 h-72 sm:h-64 lg:h-72 bg-gradient-to-b from-transparent via-black/75 to-black/100" />
      </div>
      <Container className="relative z-10 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center min-w-0 w-full">
          <div className="space-y-6 sm:space-y-9 animate-fade-in-up text-left max-w-3xl mx-0 min-w-0 w-full">
            <div className="flex justify-center sm:justify-start">
              <Badge className="relative overflow-hidden bg-white/5 text-white/90 border-white/15 backdrop-blur-md shadow-[0_0_25px_rgba(255,255,255,0.07),0_0_35px_rgba(29,167,229,0.22)] px-7 sm:px-4 py-3 sm:py-1.5 text-lg sm:text-sm before:pointer-events-none before:absolute before:inset-0 before:bg-[linear-gradient(120deg,transparent,rgba(255,255,255,0.22),transparent)] before:opacity-50 before:animate-pulse">
            ...
            <p className="text-base sm:text-lg md:text-xl text-white leading-relaxed max-w-[34ch] sm:max-w-xl mx-0">
              Combinamos tecnologia antifraude e automação para eliminar erros
              manuais, reduzir custos e aumentar a confiabilidade das suas
              rotas.
            </p>

            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-stretch sm:items-start justify-start w-full max-w-xl mx-0">
              <Button
                variant="outline"
                size="lg"
                className="gap-2 group w-full sm:w-fit justify-center text-base sm:text-lg px-5 sm:px-8 py-2.5 sm:py-3.5 border-white/30 sm:border-transparent sm:bg-brand-primary sm:hover:bg-brand-primary/90 sm:shadow-[0_0_15px_rgba(29,167,229,0.3)] sm:hover:shadow-[0_0_25px_rgba(29,167,229,0.5)]"
              ...
              <Button
                variant="outline"
                size="lg"
                className="hidden sm:inline-flex border-white/30 w-fit justify-center text-base sm:text-lg px-5 sm:px-8 py-2.5 sm:py-3.5"
            ...
            <div className="lg:hidden w-full max-w-md pt-6">
